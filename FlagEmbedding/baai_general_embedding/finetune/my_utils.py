...
from transformers import AutoModelForCausalLM, AutoModel, AutoModelForSequenceClassification, is_torch_npu_available, AutoTokenizer
...
def get_model_param_list(model_names, model_type:str):
    model_param_list = []
    for name in model_names:
        # print(f"loading {name} -----------------")
        # model = load_model(name, model_type=model_type)
        model_param_list.append(name.state_dict())
    return model_param_list


def merge_param(model_param_list: List[Dict], weights):
    new_param = {}
...
# model_names_or_paths为一个列表，包含了所有模型（已经加载好）
# 
def mix_models(model_names_or_paths, 
               model_type: str, 
               weights: List[float], 
    ...
    model = model_names_or_paths[0]
    model.load_state_dict(new_param)
    
    # 模型参数更新
    # for name, param in model.named_parameters():
    #     if name in new_param:
    #         # param.data = new_param[name].data

    #         # param.copy_(new_param[name])
    #         # param[:] = new_param[name]

    #         # updated_param = new_param[name].clone().requires_grad_(param.requires_grad)
    #         # param.data = updated_param

    #         model.state_dict()[name].copy_(new_param[name])
