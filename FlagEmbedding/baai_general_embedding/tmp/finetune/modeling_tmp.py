...
class BiEncoderModel(nn.Module):
    ...
    def inference_qp(self, qp_data):
        ...
        cur_in_ids = {
                        ...
                        # 'gist_atten_mask':qp_data['gist_atten_mask'],
                        'attention_mask':qp_data['attention_mask']
                        }
        # print("-------------------3-------------------")
        # print(cur_in_ids)
        # print(f"attenmask shape:{cur_in_ids['attention_mask'].shape}")
        results = model(**cur_in_ids,return_dict=True)
        # print(f'results here is :{results}')
        # print(f"results shape:{results.last_hidden_state.shape}")
        sum_indx = qp_data['attention_mask'].sum(dim=-1)
        gist_indx = sum_indx - 2
        indx = 0
        for i in range(results.last_hidden_state.shape[0]):
            
            gist_id = gist_indx[indx].item()
            res_embedding = results.last_hidden_state[i,gist_id,:]
            res_embedding = res_embedding.unsqueeze(0)
            if self.normlized:
                res_embedding = torch.nn.functional.normalize(res_embedding, dim=-1)
                # res_embedding = res_embedding.contiguous()
            # print(f"res_embeding:{res_embedding.shape}")
            # exit()
